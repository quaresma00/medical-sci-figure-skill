# Complete Medical SCI Figure Atlas (全图表类型全景图鉴与规范)

This reference defines the complete typology of figures found across all high-impact medical SCI publications (*NEJM*, *The Lancet*, *JAMA*, *Nature Medicine*, *Cell Press*), organized across 5 domains and 17 standardized archetypes.
...
* **Scope**: RT-qPCR, Western Blot optical density, ELISA, small animal tumor burden ($n = 3 \sim 8$).
* **Standard Visuals**: 
...
* **Scope**: Clinical cohort biomarkers, patient serum cytokines, cell culture assays ($n \ge 10$).
* **Standard Visuals**:
...
* **Scope**: Flow cytometry Mean Fluorescence Intensity (MFI), single-cell expression distribution ($n \ge 30$).
...
* **Scope**: Before vs. After treatment in the same patient (Paired $t$-test / Wilcoxon signed-rank).
...
* **Scope**: Clinical trials, oncology survival analysis (OS, PFS, RFS, DFS).
* **Mandatory Elements**:
...
* **Scope**: Preclinical in vivo drug efficacy, animal weight tracking.
...
* **Scope**: Randomized controlled trials (RCTs), Meta-analyses, cohort studies.
...
* **Scope**: Hazard ratios/Odds ratios across patient strata (age, sex, biomarker status).
* **Standard Visuals**:
...
* **Scope**: Diagnostic biomarker validation, machine learning clinical risk prediction.
...
* **Scope**: Clinical prognostic scoring systems.
...
* **Scope**: RNA-seq, proteomics, single-cell differential analysis.
* **Standard Visuals**:
...
* **Scope**: Multi-gene expression profiling, patient clustering.
...
* **Scope**: Single-cell transcriptomics (scRNA-seq), CyTOF, bulk PCA.
...
* **Scope**: Pathway enrichment (Gene Ontology, KEGG, Reactome).
...
* **Scope**: Single nucleotide polymorphism (SNP) disease association.
